...
Subroutine fillParamSensibility_GradD(AE, BE, MaxLRows, XLL, NDim, iDofT, NodELT, & 
...
!~     !   =====   END ARGUMENTS  =======
    ...
    integer :: damageEvol,constLaw, Idamage, IdamageNew, iShiftU, iShiftUd, iShiftL, iFEMtype, iMaterial
	...
	real*8 :: derCost, cost, weight1, weight2, normL2_ref, normEnergy_ref, energy, weight3, iShiftD
	integer :: Icost, IderCost, IcostNew, IderCostNew
	
	real*8 :: SolD(3), GradD(NdimE) 
	...
	Do nG = 1, NGP ! LoopGauss
		...
		!! SolD
		call PtG%calcGradTheta(GradD,SolD,nG)
	
		derCost = derCost + dot_product2(PK,GradL)*dV					
		
		Cost = Cost +  weight1 * dot_product(Udif,Udif)*DV 
		Cost = Cost + weight2 * energy * DV
		Cost = Cost + weight3 * dot_product(GradD,GradD)*DV 

		derCost = derCost - 2.0*weight3*dsqrt(dot_product(GradD,GradD))*dV
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Subroutine fillParamSensibility_GradDS(Coupling,CommonPar,iDofT,Ndim,MaxLRows,iAdd)
    !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    IMPLICIT NONE
