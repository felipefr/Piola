Subroutine FbarMaterial(AE, BE, MaxLRows, XLL, NDim, iDofT, NodElt, Sol0, Sol1, CommonPar, Param, JParam, DelT, DTm, Time)
!     ------------------------------------------------------------------
	use funcAux
	use loadingLib
	use finiteStrainLib
...
!~ 	!   =====   END ARGUMENTS  =======
	...
	call setF(F0,LoadPar,Time,DelT,LoadTypeProg,LoadType)
	F0 = F0 + deltaKron(1:NdimE,1:NdimE) + GradU
	...
	Do nG = 1, NGP ! LoopGauss
		...
		if(IdamageModify == 1 .or. IdamageModify == 5 ) then ! Coupled
			call strainEnergy(energy,Fbar,matPar,constLaw)
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Subroutine FbarMaterialS(Coupling,CommonPar,iDofT,Ndim,MaxLRows,iAdd)
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    use funcAux
    use ptsGaussLib2
    IMPLICIT NONE
