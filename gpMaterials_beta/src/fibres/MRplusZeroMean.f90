...
!     ------------------------------------------------------------------
Subroutine MRplusZeroMean(AE, BE, MaxLRows, XLL, NDim, iDofT, NodELT, Sol0, &
...
!     ------------------------------------------------------------------
	...
	integer, parameter ::  NodG = 2, NodLag = 3 , NdofLag = NdimE*NdimE + NdimE
...
!~ 	call Random_number(Areafib)
	...
	MatB(1,1) = -afib(1)*Areafib
	MatB(1,3) = afib(1)*Areafib
	MatB(2,1) = -afib(2)*Areafib
	MatB(2,3) = afib(2)*Areafib
	MatB(3,2) = -afib(1)*Areafib
	MatB(3,4) = afib(1)*Areafib
	MatB(4,2) = -afib(2)*Areafib
	MatB(4,4) = afib(2)*Areafib
	MatB(5,1) = 0.5d0*Vf
	MatB(5,3) = 0.5d0*Vf
	MatB(6,2) = 0.5d0*Vf
	MatB(6,4) = 0.5d0*Vf
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Subroutine MRplusZeroMeanS(Coupling,CommonPar,iDofT,Ndim,MaxLRows,iAdd)
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
