...
!     ------------------------------------------------------------------
Subroutine minRestrictionContinuumFibres(AE, BE, MaxLRows, XLL, NDim, iDofT, NodELT, Sol0, &
...
!     ------------------------------------------------------------------
	...
	Real*8 :: eps, pen 
	Real*8 :: afib(NdimE), Areafib, Lfib, signal, BT(NdimE,NdimE) , v(NdimE), y(NdimE), normal(NdimE), AreaFibMod
...
!~ 				
	...
	Do A=1,NodG 
		...
		call getNormalBoundary(normal,flag,y)
		AreaFibMod = Areafib/dabs(dot_product(afib,normal))
		...
		Do p = 1 , NdimE
			...
			Do q = 1 , NdofLag
				...
				if(i == p .and. flag) then
					AE(Bqp,App) = pen*signal*AreafibMod*normal(j)
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Subroutine minRestrictionContinuumFibresS(Coupling,CommonPar,iDofT,Ndim,MaxLRows,iAdd)
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	use funcAux
